...
def main(dataset, split, tfr_path, lmdb_path):
    ...
    # create lmdb
    env = lmdb.open(lmdb_path, map_size=1e11)
    count = 0
    with env.begin(write=True) as txn:
        for tf_ind in range(num_shards):
            ...
            index_path = None
            description = {'shape': 'int', 'data': 'byte', 'label': 'int', 'attr': 'int'}
            ...
            for data in loader:
                ...
                attr = data['attr'][0].cpu().numpy().astype(np.int8)
                txn.put(str(count).encode(), im)
                txn.put(('a'+str(count)).encode(), attr)
                count += 1
                if count % 100 == 0:
                    print(count)
